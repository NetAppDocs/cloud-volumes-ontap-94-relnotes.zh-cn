...
在 Azure 中，磁盘限制可防止您单独使用磁盘来达到 368 TiB 容量限制。在这些情况下，您可以通过达到 368 TiB 容量限制 https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/concept-data-tiering.html["将非活动数据分层到对象存储"^]。请参见  and disk limits by Azure VM size,容量和磁盘限制，按 Azure 虚拟机大小计算 有关详细信息：
...
| 所有聚合中的最大数据磁盘数 ^3^ | 取决于虚拟机大小。  and disk limits by Azure VM size,请参见下文。 
